    ...
    unsafe fn generate_bytes_inner(&mut self, output_slice: &mut [u8]) {
        ...
        for output_chunk in output_slice.chunks_exact_mut(BLOCK_BYTES) {
            ...
            for j in 0..2 {
                ...
                let t0_lo = _mm_alignr_epi8::<4>(s_lo, s_hi);
                let t0_hi = _mm_alignr_epi8::<4>(s_hi, s_lo);
                ...
                let t1_lo = _mm_alignr_epi8::<12>(s_hi, s_lo);
                let t1_hi = _mm_alignr_epi8::<12>(s_lo, s_hi);
